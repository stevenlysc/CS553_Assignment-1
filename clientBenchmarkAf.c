...
void *tcpClient(void *arg) 
{
	...
	for (i = 0; i < LOOPS; ++i)
	{
		send(client_sock, buffer, sizeof(char) * (thr_arg -> buf_size), 0);
		recv(client_sock, buffer, sizeof(char) * (thr_arg -> buf_size), 0);
	}
}
...
void *udpClient(void *arg) 
{
	...
	for (i = 0; i < LOOPS; ++i)
	{
		sendto(client_sock, buffer, sizeof(char) * (thr_arg -> buf_size), 0, (struct sockaddr *)&(thr_arg -> server_addr), sizeof(struct sockaddr));
		recvfrom(client_sock, buffer, sizeof(char) * (thr_arg -> buf_size), 0, (struct sockaddr *)&(thr_arg -> server_addr), &sockaddr_in_size);
	}
}
...
int main(int argc, char *argv[])
{
	...
	printf("With %d threads, the latency is %10.8f ms and the throughput is %10f Mb/S\n", num_thr, latency,  throughput);
